...
def save_chat_history(user_id: str, user_role: str,
...
def save_appointment_request(preferred_date: str, preferred_time: str,
                             reason: str, user_role: str, original_message: str, phone_number: str = None):
    ...
    # also persist to local sqlite for admin viewing (best-effort)
    if insert_appointment is not None:
        try:
            # use a copy to avoid any side-effects
            insert_appointment(dict(appointment_data))
        except Exception:
            # don't let sqlite failures break existing flow
            pass
    if db is not None and FIREBASE_INITIALIZED:
        try:
